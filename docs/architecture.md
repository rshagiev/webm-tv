...
Recipients of ordinary shared clips start watching the exact attachment as soon as it resolves, without an intermediate Watch button. That attachment pauses on its final frame instead of advancing automatically; the recipient can replay it or swipe to continue the normal feed. The same exact shared attachment also pauses at its end after going back or reloading the link. The existing explicit 18+ shared-clip confirmation remains. Selection and playback intent update together, and an aborted request cannot start a clip after a channel change. The player attempts normal playback and falls back to muted playback when the browser rejects autoplay with sound. Browser settings can still require a gesture. A shared 18+ attachment is labelled before playback and does not silently enable adult sources in the recipient's general feed. The direct attachment can be watched independently of their duration filter; subsequent recommendations retain their filters. Choosing a different channel cancels a pending shared-link request.
...
`server/share-page.ts` renders per-clip Open Graph metadata into the initial `/watch/` HTML, without requiring crawler JavaScript. The public nginx `/watch/` location must proxy to the application instead of falling back to the static index; application deployment alone does not install this configuration. Missing clips return 404 with the application shell.

`/api/preview/:board/:thread/:file` serves the exact indexed video's source JPEG thumbnail through the existing server session and allowlisted mirror redirect rules. Source thumbnail names use `{file stem}s.jpg`, verified against the supplied thread JSON. Requests are limited to eight concurrent fetches, eight seconds, and 2 MiB per image; successful responses cache for one hour. No source cookies are exposed and no video is downloaded for preview generation. If the source thumbnail is unavailable, preview returns 502. Telegram rendering and its cached messages require separate verification after deployment.
...
`excludedThreads` stores board/thread identities and display titles in this browser's localStorage. The shared metadata index and other viewers are unchanged. Exclusions filter candidate selection, refill availability and prepared media; history is pruned atomically, and a blocked current clip is stopped and cleared. Excluding a directly selected thread returns to the root feed. Reload recovery filters its saved history against current exclusions; explicit shared links also respect them. The eye button still hides one clip immediately and offers an “exclude thread” notification for 15 seconds, paused while hovered or keyboard-focused; Escape dismisses it. Desktop uses one compact row, mobile moves the exclusion action to a full-width second row. The curved undo arrow reverses the displayed action: clip hiding removes that clip from hidden/failed sets without interrupting current playback, while thread exclusion restores only the thread. Thread rows expose an explicit details menu; excluded rows stay identifiable and can be restored there or in Sources. Undoing thread exclusion leaves any earlier individually hidden clip hidden.
...
## Anonymous audience counters

`server/audience.ts` stores daily browser counts and approximate visible playback seconds in `data/audience.json` (under `WEBMTV_DATA_DIR` when set). `/api/audience` returns only aggregates for the latest 30 UTC calendar days; the info dialog refreshes today's figures every 15 seconds. This endpoint is intentionally public, like the figures in the dialog.

The client keeps a random daily identifier in `webmtv:audience` localStorage and sends a heartbeat every 15 seconds while visible. The server stores only a SHA-256 digest namespaced by its UTC day, never the raw identifier, IP, clip address or viewing preferences. Online means distinct daily identifiers with a visible heartbeat in the past 60 seconds. Closing/hiding expires naturally, so a hidden tab cannot remove another visible tab from online. Different browsers, cleared storage and storage-disabled tabs can count separately; these are browser estimates, not people or authenticated users. Existing visitors need to reload the app before they contribute.

A one-second client sampler counts wall time only when a player video advances while visible, unpaused and ready. It does not count seeks as their media-time distance. Heartbeats carry at most 30 seconds; server elapsed-time caps prevent overlapping tabs or duplicate delivery from adding parallel watch time. First contact, long gaps, midnight rotation and failed deliveries can undercount. There is no cross-day or cross-device identity and no exact unique total across multiple days. These unauthenticated client reports are not fraud-resistant analytics.

The server validates payloads and a same-origin custom header under the existing Host/Origin guard, limits each day to 100,000 identifiers, expires in-memory presence, and retains 30 daily records. Writes are serialized and atomic with 0600 permissions, every 30 seconds and on graceful shutdown. An abrupt crash can lose the last 30 seconds of collected reports. Invalid persisted data fails startup instead of silently erasing history. No third-party analytics service or video archive is involved. Local development and browser verification contribute only to the local server's file; deployment starts an independent public count.

Local verification on 2026-09-14: 49 tests and production build passed. The in-app browser reported one browser/one online for two tabs, actual advancing video added watch time, and the counter stayed at 37 seconds after pausing. The info dialog was visually checked at 390×844. Restart persistence and invalid-event rejection are covered by the API integration test. Physical iPhone/Safari and the public deployment have not been verified for this change.

## Thread focus toggle

The outlined “Смотреть этот тред” control is visible on desktop and mobile. It is gray in the broader feed and orange with a checkmark and “Смотрим этот тред” when the selected source matches the current clip's thread. Toggling on preserves the current clip and remembers the previous source set; toggling off restores that set while keeping playback on the current clip. A directly opened thread, or a reloaded page without an in-memory return source, falls back to the root feed. Selecting another source clears the return target. The toggle replaces the separate return-stream link.

...
На телефоне нажмите верхнюю кнопку справа — **Поделиться**. Откроется системное меню отправки со ссылкой на этот ролик в WebM TV. Друг сможет посмотреть его и продолжить листать эфир. Если системное меню недоступно, ссылка копируется или показывается для ручного копирования. Видео не архивируются: удалённый у источника ролик может перестать открываться.
...
**Нужно заранее скачивать коллекцию?** Нет. Сервер постепенно индексирует ссылки и метаданные в `data/`, без настроенной сессии видео идут напрямую с источника; с сессией — потоком через сервер WebM TV. При холодном запуске дерево наполняется постепенно.
...
**Почему скрытая доска не загружается?** Двач может требовать сессию, полученную после отправки поста. Когда общий доступ пропадает, WebM TV предлагает «Помочь восстановить». Один зритель решает капчу и явно отправляет `test` в тестовом разделе Двача. Сервер получает новую сессию, проверяет закрытый каталог и сохраняет доступ для всех; личная cookie зрителя не нужна. При общем сбое Двача приложение предлагает подождать. Восстановление также доступно по пути `/restore-access`.

Для ручной настройки серверной сессии можно сохранить только значение `usercode_auth` в `data/source-usercode.txt` (альтернативный путь — `SOURCE_USERCODE_FILE`). Файл не входит в Git; доступ к нему должен быть только у пользователя сервера. Замену сервер подхватит при следующем запросе. В публичной установке найденные видео поступают в общий индекс, а не в личный каталог владельца cookie.
